...
左側に整数または整数式が指定されていて、右側に整数式が指定されている場合は、`<<<` (算術左シフト) 演算子または `>>>` (算術右シフト) 演算子を使用して、左側と同じ型の新しい式を作成できます。条件.
...
2つの式が等しい場合、式は true になります (resp ではありません)。

ユーザー定義型の値を比較することはできません。値のみを比較できます。 たとえば、次のように入力します。
...
2つの数値式を指定した場合、2項演算子 `>`、`<`、`>=`、および `<=` を使用して新しいブール式を作成することができます。この式は、最初の式がそれぞれ次の値より大きい、より小さい、または等しい場合に true になります。、または2番目の式以下。
...
`start`、`step`、および `stop`の3つの `Int` 式が指定されている場合、`start .. step .. stop` が渡されるまで、最初の要素が `start`、2番目の要素が `start+step`、3番目の要素が `start+step+step`などの範囲式です。
...
たとえば、`Func` にシグネチャ `('T1, 'T2, 'T1) -> 'T2`がある場合、`Op1` と `Op2` に署名 `(Qubit[] => Unit is Adj)`があり、`Op3` を1番目の引数として `(Qubit[] => Unit)`を呼び出すには、次のように `Func` します。、および `Op3` 3 番目のとします。
...
たとえば、`Op1`、`Op2`、および `Op3` はすべて `Qubit[] => Unit`ですが、`Op1` は `Adjoint`をサポートし、`Op2` は両方をサポートします。
...
Operator | アリ | description | オペランドの型
---------|----------|---------|---------------
 `!` の末尾 | 前置 | ラップ解除 | 任意のユーザー定義型
 `-`、`~~~`、`not` | 前置 | 負の数値、ビットごとの補数、論理否定 | `Int` の `-`、`BigInt`、`~~~`の `Int`、`BigInt` または `Double` `Bool` の `not`
