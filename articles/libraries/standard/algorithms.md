...
振幅増幅の背後にある中心となる考え方は、一連の反射を行うことによって、目的の結果が発生する確率を増幅することです。  これらの反射は、初期状態を目的の状態 (マークされた状態と呼ばれることもあります) の近くに向かって回転させます。  具体的には、初期状態がマーク付きの状態になる確率が $ \ sin ^ 2 (-シータ) $ である場合、振幅増幅を適用した後 $m $/sin ^ 2 ((2m + 1) \ シータ) $ になります。  これは、$ \ シータ = \ pi/[2 (2n + 1)] $ $n の値がある場合に $ を使用すると、振幅増幅の $n $ 反復の後、$100\\% $ に成功する確率を上げることができることを意味します。  $-シータ = \ sin ^{-1}(\ sqrt{/Pr (success)}) $ であるため、成功を取得するために必要なイテレーションの数は、ランダムに設定された、無作為に指定された状態を検出するために必要な数よりも少なくなります。サンプリング.

振幅増幅の各反復処理では、2つのリフレクション演算子が指定されている必要があります。 具体的には、$Q $ が振幅増幅で反復処理され、$P 0 $ がプロジェクターの最初のサブ空間にあり、$P _1 $ が、マークされたサブ空間上のプロジェクターであることを示します。 $Q その後、=-(\ bold one25) (\ bold done-2P_1) $ となります。  プロジェクターは、固有値 $ + $1 および $0 $ を持つ Hermitian 演算子で、結果として $ (& a) $ (& a) $ (& a) $ (& a) は、unity (この場合は $ pm $1) のルートである固有値を持つため、"$ (& a)" となります。 例として、初期状態 $H ^ {\ otimes n} \ket{0}$ とマークされた状態 $ \ket{m} $、$P (0 = H ^ {/otimes n} \ket{0}\bra{0}H ^ {/otimes n} $、および $P _1 = \ket{m}\bra{m} $ という形式の Grover の検索を使用する場合を考えてみます。  ほとんどのアプリケーションでは、$P 0 $ はプロジェクターの初期状態になります。これは、一部の vector $ \ket{\psi} $; に対して $P 0 = k {\ psi} $ が設定されていることを意味します。ただし、無関係振幅 $P amplication では、通常、0 $ は多くのクォンタム状態に投影されます (つまり、$ + $1 eigenvalue of $P 0 $ の複数要素の接続性は $1 $ を超えています)。

振幅増幅の背後にあるロジックは、$Q $ の eigen 分解から直接たどります。  具体的には、初期状態がゼロ以外のサポートを持つ $Q $ の固有ベクトルは、$P 0 $ と $P _1 $ の $ + $1 固有ベクトルの線形組み合わせとして表示できます。  具体的には、振幅増幅の初期状態 ($P ($ + $1 eigenvector) であることを前提として、$ $ \ket{\psi} = \frac{-i}{\sqrt{2}} \ left (e ^ {i} \ k {\ psi_ +} + e ^ {-} \ k {\ psi_} \ right)、$ $ where $ \ket{\psi_\pm として記述できます。} $ は $Q $ with 固有値 $e ^ {\ pm 2i \ シータ} $ で、$ + $1 $P 固有ベクトル (0 $ および $P _1 $) でのみサポートされています。  固有値が $e ^ {\ pm i > $ の場合、演算子 $Q $ は、2つのプロジェクターによって指定された2次元のサブ空間で回転を実行し、回転角度が $ 2 \ シータ $ である初期状態を意味します。  このため、$ iteration $m $ iteration $Q $ の成功確率は $ \ sin ^ 2 ([2m + 1] \ シータ) $ です。
...
一般ルーチン (`AmpAmpObliviousByReflectionPhases`) には、`ancillaRegister` と `systemRegister`を呼び出す2つのレジスタがあります。 また、必要な反射に対して2つの oracles も受け入れます。 `ReflectionOracle` は `ancillaRegister` でのみ動作し、`ObliviousOracle` は両方のレジスタに共同で作用します。 `ancillaRegister` への入力は、最初のリフレクション演算子 $-bold done-2P_1 $ の-1 eigenstate に初期化する必要があります。
...
おおよその QFT には、dyadic $Z $ ローテーション操作 <xref:microsoft.quantum.primitive.rfrac> および <xref:microsoft.quantum.intrinsic.h> 操作が必要です。
...
$K \ ge \log_2 (n) + \log_2 (1/-イプシロン) + $3 であることがわかっています。 $\\| をバインドできます。\ オペレーター名 {qft}-& # {\\|</イプシロン $。
...
このメソッドは、$ $ \operatorname{Add}\ket{a}\ket{b} = \ket{a}\ket{a + b}、$ $ $n を介して2つのクォンタム入力を持つ追加を行うことができます。これは、\_& \begin{align} の形式で制御されるアプリケーションを使用します。\_0} \ 左 (\ オペレーター名 {add} (1) \ 右) \ ラムダ\_{a\_1} \ 左 (\ オペレーター名 {add} (2) \ 右) \ ラムダ\_{a\_2} \ 左 (\ 演算子名 {add} (4) \ 右) \ cドット & ラムダ\_{a\_{n-1}} \ 左 (\オペレーター名 {Add} ({{n-1}}) \ right) \ket{a}\ket{b} \\\\ & = \ket{a} \ket{b + a} $n、$ ビット整数 $a $ および $b $ および加算剰余 $ 2 ^ n $。  $-ラムダ\_x (A) $ という表記では、$ という表記では、操作の制御されたバージョンに $A 対して、コントロールとして qubit $x $ が指定されていることを思い出してください。

同様に、クラシックデプロイで制御される乗算 (Shor のファクタリングアルゴリズムに不可欠なモジュール形式の形式) は、\begin{align} \operatorname{Mult} (a) \ket{x}\ket{b} & = \ ラムダという同様の一連の制御された追加を使用して実行できます。\_{x\_0} \ left (\ 演算子 name{add} (2 ^ 0 a) \ right) \ ラムダ\_{a\_1} \ 左 (\ 演算子名 {add} (2 ^ 1a) \ right) \ ラムダ\_{a\_2} \ 左 (\ オペレーター名 {add} (2 ^ 2 a) \ 右) \\_{x\_{n-1}} \ left (\ 演算子 name{add} ({2 ^ {n-1}} a) \ right) \ket{x}\ket{b} \\\\ & = \ket{x}\ket{b + ax}。
...
$ $ ($ $ \Ket{\phi\_k (a)} = \ frac{1}{/sqrt{2}} \ left (\ket{0} + e ^ {i2\pi a/2 ^ k} \ket{1}) を定義すると、$ $ \operatorname{QFT}\ket{a} = \ket{\phi\_1 (a)} \ otimes/cドット \ が表示されます。otimes \ket{\phi\_n (a)}。
...
この拡張は、任意の整数 $j $ と実数 $x $、$e ^ {i2\pi (x + j)} = e ^ {i2\pi x} $ に対してさらに簡略化できます。  これは、円で $ 360 ^ {\circ} $ 度 ($ 2 \ pi $ ラジアン) を回転させると、開始位置が正確になるためです。  このため、$x $ for $e ^ {i2\pi x} $ の唯一の重要な部分は $x $ の小数部分です。  具体的には、$x = y +0\_0x\_2 \ ldots x\_n $ then $e ^ {i2\pi x} = e ^ {i2\pi (0 .x\_0x\_2 \ ldots x\_{n-1})} $ と $ $ \ket{\ という形式のバイナリ展開があるとします。phi\_k (a + b)} = \ frac{1}{\ sqrt{2}} \ left (\ket{0} + e ^ {i2\pi [a/2 ^ k +0\_k\ \ket b\_1]}{1}/right). $ $ これは、それぞれを増分して追加を実行すると、$ \ket{a} $ のフーリエ変換を拡大すると、回転数が $ 減少 $k に縮小されます。  これにより、増加に必要なクォンタムゲートの数が大幅に削減されます。  ここでは、フーリエ変換、フェーズの追加、および逆フーリエ変換のステップを示しています。これは、$ Draper の追加を構成します。この手順では、$-の{-1} 演算子として、$-(\ phi\\\!。 この単純化を使用してプロセス全体を実装するクォンタム回線は、次のようになります。
...
### <a name="quantum-phase-estimation"></a>クォンタムフェーズの推定 ###
...
$V $ が $U $ の制御されたアプリケーションであるとします。たとえば、\begin{align} V (\ket{0} \ otimes \ket{\phi}) & = \ket{0}/otimes \ket{\phi} \\\\ \t extrm{および} V (\ket{1}/otimes \ket{\phi}) & = e ^ {i \phi} \ket{1} \ otimes \ket{\phi}.
\end{align}、直線性、\begin{align} V (\ket{+}、otimes \ket{\phi}) & = \frac{(\ket{0} \ otimes \ket{\phi}) + e ^ {i \phi} (\ket{1}/otimes \ket{\phi})} {/sqrt{2}}。
\end{align} \begin{align} V (\ket{+}/otimes \ket{\phi}) & = \frac{\ket{0} + e ^ {i \phi} \ket{1}} {\ sqrt{2}}/otimes \ket{\phi} \\\\ & = (R_1 (+ phi) \ket{+}) \ otimes \ket{\phi} を見つけるための用語を収集できます。,、\end{align} _1 $ $R <xref:microsoft.quantum.intrinsic.r1> 操作によって適用されるユニタリです。
...
この方法で続行します。この形式の登録を取得するには、\begin{align} \ket{\psi} & = R_1 (2 ^ j/phi) \ket{+} \\\\ &/propto/bigj = 0} ^ {n} \ left (\ket{0} + \ exp (i 2 ^ {j}/phi) \ket{1}右) を使用し \\\\ & \ prop$n 0} ^ {2 ^ n-1} \ exp (i-phi k) \ket{k} \end{align}。ここで、$ は必要な精度のビット数で、${}/propto {}$ を使用して、正規化係数 $ を抑制したことを示しています。1/\ sqrt{2 ^ n} $。

$/Phi = 2-pi p/2 ^ k $ が整数 $p $ であると仮定した場合、これは $ \ket{\psi} = \ket{p_0 p_1/ドット p_n} $ として認識されます。ここで、$p j $ は $j ^ {\t extrm{th{1}} $ bit of $2 $ bit です。
