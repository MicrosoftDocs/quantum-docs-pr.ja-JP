...
このような変更の1つの例として、ユーザー定義の特殊化のシンボルの組があります。 以前のリリースでは、制御された、または制御された adjoint 特殊化宣言で1つのシンボル引数が指定されていましたが、0.3 以降では、コンパイラを使用していることを示す `...` のすべての特殊化にこのようなシンボルタプルを指定することを推奨は、必要に応じて、呼び出し可能な宣言からシンボルを移行する必要があります。 
