...
Quantum 開発キットの最新の更新プログラムを紹介します。  このページでは、新しい Q # 言語機能の概要を示し、既存の Q # プログラムを新しい構文に[移行](#Migration)する方法について説明します。  Q # 言語の詳細については、[こちら](xref:microsoft.quantum.guide.basics)の完全なドキュメントを参照してください。
...
条件式は、という形式の式です `condition ? caseTrue | caseFalse` 。
上の例からわかるように、配列に対する反復処理もサポートされるようになりました。

と内の割り当て `using` `borrowing` は、1つの qubits、qubits 配列、および入れ子になった組で新しく可能になります。
```qsharp
    using (qubit = Qubit()) {
...
に加えて、2つの新しい特殊化ジェネレーターディレクティブ `invert` と `distribute` が追加され、 `auto` ファンクタの特殊化の生成方法を制御できるようになりました。
操作内の既定の特殊化の明示的な宣言は、必須ではなくなりました `body` 。
...
修飾子は、他の式連結子よりも厳密にバインドされ、特に、呼び出し式の連結子よりも厳密に `(` `)` なります。 
`Adjoint`、 `Controlled` 、および新しい "ラップ解除" 後置演算子 `!` は、式修飾子として扱われます。 `Adjoint`And を `Controlled` 式修飾子として扱うことにより、多くの場合にかっこを使用する必要がなくなります。 たとえば、次の式は、の前に進むことができます。また、一連の処理には、次のような処理が `Op` `opArr` `arg` あります。 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
移行スクリプトを実行すると、不要なかっこの大半が削除され、新機能を最大限に活用できます。 ただし、スクリプトではコードが解釈されず、ほとんどの場合、ファンクタの異なる解釈が考慮されないことに注意して `Controlled` ください (部分的なアプリケーションとの組み合わせのみに関係します)。

新しい修飾子は、 `!` ユーザー定義型を基になる型にキャストします。 このキャストは、以前のリリースではアップキャストと見なされており、自動的に実行されました。 ユーザー定義型の新しい解釈を使用すると、このようなことはなくなり、キャストを明示的に行う必要があります。    
...
```
例からわかるように、では、やなどの `!` プレフィックス修飾子よりも厳密にバインドさ `Controlled` れて `Adjoint` います。

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>重大な変更 
...
次に示す変更に加えて、 `__*__` * は空白以外の文字の任意のシーケンスであり、内部使用のために予約されており、シンボル名として使用することはできません。  
...
このような変更の1つの例として、ユーザー定義の特殊化のシンボルの組があります。 以前のリリースでは、制御された、または制御された adjoint 特殊化宣言は1つのシンボル引数を指定していましたが、0.3 以降では、必要に応じてコンパイラが呼び出し可能な `...` 宣言からシンボルを移行することを示すために、すべての特殊化にこのような記号のタプルを提供することをお勧めし 

## <a name="semantic-changes"></a>セマンティックの変更 

0.3 リリースの最もインパクトな中断は、ユーザー定義型を型システムに統合する方法です。 以前のリリースでは、ユーザー定義型はその基になる型のサブタイプと見なされていました。 今回のリリースでは、今後この問題が発生することはありません。 ユーザー定義型は、独自の別個の型と見なされるようになりました。ユーザー定義型とその基になる型の間の自動キャストは存在しません。 すべてのキャストは、"ラップ解除" 演算子を使用して明示的に行う必要があり `!` ます。また、特にユーザー定義型のオブジェクトのコンテンツにアクセスするには、キャストが必要です。  
...
次の例は、関数を使用して関数を操作に変更する方法を示してい `ToOperation` `Square` `op` ます。 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>既存のプロジェクトを Q # 0.3 に移行する 
...
2. テキストエディターで、.csproj ファイルを編集して、すべての "0.3.1811.2802" のバージョン `PackageReference` をバージョン-preview に変更します。次に例を示します。
```xml
...
4. コマンドラインから、コマンドラインコンパイラに統合された書式設定ツールを使用して、次のコマンドを実行してすべての構文の変更に対処します。`dotnet msbuild /t:qsharpformat`  
    - ファイルはインプレースで移行されます。 すべての元のファイルのバックアップがにコピーされます。`obj\qsharp\.backup`
...
## <a name="migration-script"></a><a name="MigrationScript"></a>移行スクリプト 
...
- このプロジェクトは、移行が成功する可能性があることを確認するために、以前のバージョンの Quantum 開発キットでビルドされています。 この手順は、スクリプトのパラメーターを使用してスキップでき `-Force` ます。
...
> などのスクリプトを Windows で実行できるようにするには `qdk-migrate.ps1` 、実行ポリシーの変更が必要になることがあります。
