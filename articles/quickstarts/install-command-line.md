---
title: 'IDE で :::no-loc(Q#)::: アプリケーションを使用して開発する'
description: 'コマンド プロンプトから実行される :::no-loc(Q#)::: アプリケーションを作成する方法について説明します。'
author: bradben
ms.author: v-benbra
ms.date: 8/20/2020
ms.topic: article
...
---
# <a name="develop-with-no-locq-applications-in-an-ide"></a><span data-ttu-id="8d8c7-103">IDE で :::no-loc(Q#)::: アプリケーションを使用して開発する</span><span class="sxs-lookup"><span data-stu-id="8d8c7-103">Develop with :::no-loc(Q#)::: applications in an IDE</span></span>

<span data-ttu-id="8d8c7-104">:::no-loc(Q#)::: プログラムはそれ自体で実行可能です。C#、F#、Python などのホスト言語のドライバーは必要ありません。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-104">:::no-loc(Q#)::: programs can run on their own, without a driver in a host language like C#, F#, or Python.</span></span> <span data-ttu-id="8d8c7-105">Visual Studio Code (VS Code)、Visual Studio、Visual Studio Codespaces、または任意のエディター/IDE で :::no-loc(Q#)::: アプリケーションを開発し、.NET コンソールからアプリケーションを実行できます。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-105">You can develop :::no-loc(Q#)::: applications in Visual Studio Code (VS Code), Visual Studio, Visual Studio Codespaces, or with any editor/IDE and run applications from the .NET console.</span></span> 
...
<span data-ttu-id="8d8c7-109">:::no-loc(Q#)::: アプリケーションはどの IDE でもビルドできますが、:::no-loc(Q#)::: アプリケーションをローカルで開発する場合は Visual Studio Code (VS Code) または Visual Studio IDE を使用することをお勧めします。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-109">While you can build :::no-loc(Q#)::: applications in any IDE, we recommend using Visual Studio Code (VS Code) or Visual Studio IDE for developing your :::no-loc(Q#)::: applications locally.</span></span> <span data-ttu-id="8d8c7-110">Web ブラウザーを使用してクラウドで開発する場合は、Visual Studio Codespaces をお勧めします。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-110">For developing in the Cloud via the web browser, we recommend Visual Studio Codespaces.</span></span> <span data-ttu-id="8d8c7-111">これらの環境での開発では、警告、構文の強調表示、プロジェクト テンプレートなど、QDK 拡張機能の豊富な機能を利用できます。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-111">Developing in these environments leverages the rich functionality of the QDK extension, which includes warnings, syntax highlighting, project templates, and more.</span></span> 
...
## <a name="develop-with-no-locq"></a><span data-ttu-id="8d8c7-127">:::no-loc(Q#)::: を使用した開発</span><span class="sxs-lookup"><span data-stu-id="8d8c7-127">Develop with :::no-loc(Q#):::</span></span>
...
1. <span data-ttu-id="8d8c7-131">**[表示]**  ->  **[コマンド パレット]** をクリックして、 **[:::no-loc(Q#):::: 新しいプロジェクトの作成]** を選択します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-131">Click **View** -> **Command Palette** and select **:::no-loc(Q#):::: Create New Project**.</span></span>
...
<span data-ttu-id="8d8c7-135">プロジェクトを確認します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-135">Inspect the project.</span></span> <span data-ttu-id="8d8c7-136">`Program.qs` という名前のソース ファイルがあるはずです。これは、コンソールにメッセージを表示する簡単な演算を定義する :::no-loc(Q#)::: プログラムです。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-136">You should see a source file named `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span>
...
> [!NOTE]
> <span data-ttu-id="8d8c7-141">複数のルート フォルダーを持つワークスペースは、VS Code の :::no-loc(Q#)::: 拡張機能では現在サポートされていません。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-141">Workspaces with multiple root folders are not currently supported by the VS Code :::no-loc(Q#)::: extension.</span></span> <span data-ttu-id="8d8c7-142">1 つの VS Code ワークスペース内に複数のプロジェクトがある場合は、すべてのプロジェクトが同じルート フォルダー内に含まれている必要があります。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-142">If you have multiple projects within one VS Code workspace, all projects need to be contained within the same root folder.</span></span>
...
<span data-ttu-id="8d8c7-144">:::no-loc(Q#)::: の `Hello World` アプリケーションを作成して、Visual Studio のインストールを確認します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-144">Verify your Visual Studio installation by creating a :::no-loc(Q#)::: `Hello World` application.</span></span>

<span data-ttu-id="8d8c7-145">新しい :::no-loc(Q#)::: アプリケーションを作成するには:</span><span class="sxs-lookup"><span data-stu-id="8d8c7-145">To create a new :::no-loc(Q#)::: application:</span></span>
...
2. <span data-ttu-id="8d8c7-147">検索ボックスに「`:::no-loc(Q#):::`」と入力し、 **[:::no-loc(Q#)::: アプリケーション]** を選択して **[次へ]** をクリックします。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-147">Type `:::no-loc(Q#):::` in the search box, select **:::no-loc(Q#)::: Application** and click **Next**.</span></span>
...
<span data-ttu-id="8d8c7-149">プロジェクトを確認します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-149">Inspect the project.</span></span> <span data-ttu-id="8d8c7-150">`Program.qs` という名前のソース ファイルがあるはずです。これは、コンソールにメッセージを表示する簡単な演算を定義する :::no-loc(Q#)::: プログラムです。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-150">You should see a source file named `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span>
...
<span data-ttu-id="8d8c7-156">:::no-loc(Q#)::: の `Hello World` アプリケーションを作成して、インストールを確認します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-156">Verify your installation by creating a :::no-loc(Q#)::: `Hello World` application.</span></span>
...
1. <span data-ttu-id="8d8c7-158">アプリケーション ディレクトリに移動します。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-158">Navigate to the application directory:</span></span>
    ...
    <span data-ttu-id="8d8c7-159">このディレクトリには `Program.qs` という名前のファイルがあるはずです。これは、コンソールにメッセージを表示する簡単な演算を定義する :::no-loc(Q#)::: プログラムです。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-159">This directory should now contain a file `Program.qs`, which is a :::no-loc(Q#)::: program that defines a simple operation to print a message to the console.</span></span> <span data-ttu-id="8d8c7-160">このテンプレートをテキスト エディターで変更し、独自のクォンタム アプリケーションで上書きすることができます。</span><span class="sxs-lookup"><span data-stu-id="8d8c7-160">You can modfiy this template with a text editor and overwrite it with your own quantum applications.</span></span> 
