...
<span data-ttu-id="b3e3d-272">各 :::no-loc(Q#)::: ソースファイルでは、任意の数の関数を定義できます。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-272">Each :::no-loc(Q#)::: source file can define any number of functions.</span></span>
...
<span data-ttu-id="b3e3d-299">:::no-loc(Q#)::: 関数と演算に *型パラメーター* と、通常のタプルパラメーターを設定できるようにすることで、この概念を形式化します。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-299">:::no-loc(Q#)::: formalizes this notion by allowing functions and operations to have *type parameters* , as well as their ordinary tuple parameters.</span></span>
...
<span data-ttu-id="b3e3d-344">:::no-loc(Q#)::: callables は、直接的または間接的に再帰的に許可されます。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-344">:::no-loc(Q#)::: callables are allowed to be directly or indirectly recursive.</span></span>
...
- <span data-ttu-id="b3e3d-349">実際のクォンタムデバイスで実行されている場合、スタック領域が制限される可能性があるため、詳細な再帰によって実行時エラーが発生する可能性があります。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-349">When running on an actual quantum device, stack space might be limited, and so deep recursion can lead to a runtime error.</span></span>
  <span data-ttu-id="b3e3d-350">具体的には、 :::no-loc(Q#)::: コンパイラとランタイムは、末尾の再帰を識別して最適化しません。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-350">In particular, the :::no-loc(Q#)::: compiler and runtime do not identify and optimize tail recursion.</span></span>
...
<span data-ttu-id="b3e3d-352">の [変数](xref:microsoft.quantum.guide.variables) について説明 :::no-loc(Q#)::: します。</span><span class="sxs-lookup"><span data-stu-id="b3e3d-352">Learn about [Variables](xref:microsoft.quantum.guide.variables) in :::no-loc(Q#):::.</span></span>